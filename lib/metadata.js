...
var Metadata = (function () {
    ...
    Metadata.prototype.createProperty = function (entityFullName, entitySchema, propertySchema) {
        ...
        containsField(propertySchema, "storeGeneratedPattern", function (v) {
            if (v == "Identity") {
    ...
    Metadata.prototype.createNavigationProperty = function (entityFullName, entitySchema, propertySchema) {
        ...
        containsField(propertySchema, "relationship", function (p) {
            ...
            for (var i = 0; i < entitySchema.parent.entityTypes.length; i++) {
                ...
                if (entityType == entitySchema)
                    continue;
                inverse = entityType.navigationProperties.filter(function (prop) { return prop.relationship == p && prop.fromRole == propertySchema.toRole; })[0] || null;
                if (inverse)
                    break;
            }
            for (var i = 0; i < entitySchema.parent.associations.length; i++) {
                ...
                if (association && association.name == name) {
                    var end = association.ends.filter(function (end) { return end.role == propertySchema.toRole; })[0];
                    if (end) {
                        if (end.multiplicity == "*") {
            ...
            if (inverse)
                definition.inverseProperty = inverse.name;
        });
