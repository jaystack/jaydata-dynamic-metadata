...
var Metadata = (function () {
        ...
        types.push.apply(types, typeDefinitions.map(function (d) {
            ...
            if (d.baseType == '$data.Enum') {
                ...
                if (d.params[3] && Object.keys(d.params[3]).length > 0) {
                    Object.keys(d.params[3]).forEach(function (dp) { return dtsPart.push('        static ' + d.params[3][dp].name); });
                }
    ...
    Metadata.prototype._typeToTS = function (type, elementType, definition) {
        ...
        if (type == this.options.entitySetType) {
            ...
            return '{ (' + (definition.params.length > 0 ? definition.params.map(function (p) { return p.name + ': ' + _this._typeToTS(p.type, p.elementType, p); }).join(', ') : '') + '): Promise<' + this._typeToTS(definition.returnType, definition.elementType, definition) + '>; }';
        }
        else
            return type;
    };
