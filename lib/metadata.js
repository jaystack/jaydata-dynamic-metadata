...
var Metadata = (function () {
    ...
    Metadata.prototype.processMetadata = function (createdTypes) {
        ...
        types.dts = '/*//////////////////////////////////////////////////////////////////////////////////////\n' +
            ...
            '//////                      OData  V4  TypeScript                              /////////\n' +
            '//////////////////////////////////////////////////////////////////////////////////////*/\n\n';
        types.dts += Object.keys(dtsTypeMapping).map(function (t) { return 'type ' + t + ' = ' + dtsTypeMapping[t] + ';'; }).join('\n') + '\n\n';
        var self = this;
    ...
    Metadata.prototype._typeToTS = function (type, elementType, definition) {
        ...
        if (type == this.options.entitySetType) {
            ...
            return '{ (' + (definition.params.length > 0 ? definition.params.map(function (p) { return p.name + ': ' + _this._typeToTS(p.type, p.elementType, p); }) + '; ' : '') + 'handler?: () => void): $data.IPromise; }';
        }
        else if (type == '$data.ServiceFunction') {
            return '{ (' + (definition.params.length > 0 ? definition.params.map(function (p) { return p.name + ': ' + _this._typeToTS(p.type, p.elementType, p); }) + '; ' : '') + 'handler?: () => ' + this._typeToTS(definition.returnType, definition.elementType, definition) + '): $data.IPromise; }';
        }
        else
            return type;
    };
